...
@transform(
    ...
    scooters=Input("/Atamer Systems-976c6b/Crisis Logistics Command System/02_clean_derived/lime_scooter_positions"),
    output=Output("ri.foundry.main.dataset.a774feec-e2a6-4f7e-8d14-279efe3f34ac"),
)
def compute(airlabs, bikepoints, buses, scooters, output):
    ...
    # Read Lime scooter positions
    scooters_df = scooters.polars()
    if scooters_df.height > 0:
        scooters_cols = scooters_df.select([
            pl.col("unit_id"),
            pl.col("vehicle_type"),
            pl.col("latitude"),
            pl.col("longitude"),
            pl.col("unit_status").alias("status"),
            pl.col("last_seen_at"),
        ])
    else:
        scooters_cols = pl.DataFrame({
            "unit_id": pl.Series([], dtype=pl.Utf8),
            "vehicle_type": pl.Series([], dtype=pl.Utf8),
            "latitude": pl.Series([], dtype=pl.Float64),
            "longitude": pl.Series([], dtype=pl.Float64),
    ...
    # Union all sources
    unified = pl.concat([aircraft_cols, bikes_cols, buses_cols, scooters_cols], how="vertical_relaxed")
